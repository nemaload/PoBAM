...
Because the scattering properties change over time, this must be done quickly, possibly faster than the imaging frame rate, necessitating high-speed wavefront modulation. This can currently be achieved with digital micro-mirror devices (DMDs), but not with the phase-only spatial light modulators (SLMs) that are used to prevent power losses in the excitation pathways for nonlinear microscopies, although GHz switching of phase-only modulators appears feasible in principle \cite{alivisatos13}. High speed focusing through turbid media is also achievable using all-optical feedback in a laser cavity~\cite{Nixon2013}, and it is even possible to measure the scattering matrix non-invasively \cite{Chaigne2013} using a photo-acoustic microscopy based on focused ultrasound. 

When using short optical pulses, scattering can lead to temporal distortions that significantly degrade the peak light intensity at a focal spots. The \SI{<100}{\femto\second} pulse durations used in two-photon microscopy, for example, are comparable to the time it takes light to travel \SI{30}{\micro\meter} in vacuum, and to the timescales of photo-chemical isomerization reactions in optogenetic proteins \cite{schoenlein1991first}. Fortunately, wavefront shaping techniques can correct for scattering-induced temporal distortions as well \cite{mccabe2011spatio, katz2011focusing}.
