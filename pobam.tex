...
For optical approaches the light originating from the activity of each neuron must be separated from emissions originating from other points in the brain.
Epi-fluorescence microscopy images a plane in the specimen onto a spatially-resolved two-dimensional detector (e.g., a CCD camera). The focal plane is then scanned in order to reconstruct 3D images. However, because the an entire 3D volume is illuminated during image aquisition, out-of-focus neurons serve as a source of background noise. At least four  optical techniques have been employed to reduce this source of noise: (1) Light sheet imaging is similar to epi-flourescent imaging, except that only neurons near the focal plane are illuminated. Unfortunately, this requires transparent brains~\cite{ahrens13}. (2) Volumetric imaging can also be performed in a single snapshot using lightfield microscopes \cite{levoy2009recording, broxton2013wave}, which capture the directions of incoming light rays, trading in-plane resolution for axial resolution, or by using multifocus microscopes \cite{abrahamsson2012fast}. (3) 
In multi-photon microscopy, nonlinearities result in fluorescence excitation occurring only near the focal point of the excitation laser, which is scanned across the sample. Finally (4) In confocal scanning microscopy, only photons from a point of interest are measured due to geometric constraints (e.g., pinholes).
...
\item Emerging techniques based on beam shaping could allow transmission of focused light through random scattering media by inverting the scattering matrix~\cite{conkey12}.
...
The real-time transmission bandwidth requirements for neural recording could be significantly reduced if it is only desired to take a ``snapshot'' of neural activity patterns over a limited period of time, but would require a large amount of local storage. For example, flash memory, which will likely be the densest form of electronic memory storage in the near future, can store \SI{1}{\mega\bit} of data in a device \SI{100}{\micro\meter} on a side.
