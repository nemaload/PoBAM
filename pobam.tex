...
These calculations assuming perfect spike-sorting will greatly underestimate the required number of electrodes in practice. Sorting spikes from all of the $> 500$ neurons within the \SI{3e6}{\micro\meter\cubed} cube corresponding to a single electrode is likely to be unrealistic for several reasons.
...
As discussed above, the previous calculations assume that any spike which is visible above the noise on at least one electrode can be detected and correctly assigned to a cell – that the problem of spike sorting can be solved perfectly.  However, perfect spike sorting is far beyond current capabilities and in fact may not even be possible in principle.  To achieve the $N=\num{130000}$ scenario described above would require each electrode to sort spikes from \num{\ca 500} neurons out to a distance of $r_0 = \SI{130}{\micro\meter}$.  \num{50}$\%$ of these neurons will lie at $>\SI{100}{\micro\meter}$ distance from the electrode and will therefore have $\text{SINR} < \num{2.5}$ on the electrode if we assume that spikes decay exponentially in space (see above).  According to the Shannon-Hartley theorem, the information capacity $C$ of a single analog channel (e.g. our electrode) is:

\[C = B \log_2 (1 + S/N)\]

where $B$ is the bandwidth, $S$ is signal power ($\propto$ voltage squared), and $N$ is noise power.  At $B \approx \SI{10}{\kilo\hertz\per\second}$ and $S/N \approx \num{2.5}^{\num{2}}$, the channel capacity is $C \approx \SI{30}{\kilo\bit\per\second}$, whereas to transmit uniquely decodable spike information from all \num{250} neurons at millisecond precision requires $\SI{1}{\kilo\bit\per\second} \times 250 = \SI{250}{\kilo\bit\per\second}$, a factor $\num{10}\times$ greater than the channel capacity.  Even with optimal compression of \SI{5}{\hertz} spikes (see section 2) we would need \SI{\ca 10}{\kilo\bit\per\second}, which is possible but barely so.  Thus, based on these rough estimates, perfect spike sorting at \num{500} neurons/electrode on a sphere of \SI{130}{\micro\meter}, at noise levels of current electrodes may not be possible – in essence, there may not be enough room on the electrode’s voltage trace to discriminate such a large number of weak signals.  Note that these information-theoretic limits apply even if it is possible to resolve temporally overlapping spikes, and, in fact, the channel capacity is what limits our ability to resolve such spikes.

Even if each electrode is only responsible for sorting \num{100} neurons, then $r_0 \approx \SI{75}{\micro\meter}$, $S/N < 10$ for the outer \num{50}$\%$ of neurons, and channel capacity is thus \SI{70}{\kilo\bit\per\second}, whereas \SI{50}{\kilo\bit\per\second} is needed without compression and \SI{\ca 2}{\kilo\bit\per\second} with compression, meaning that we may still be relatively close to the limits of information transmission.

In practice these numbers are likely to be highly optimistic, since the set of spikes from different neurons are very far from an optimally designed code from the perspective of multiplexed voltage signals on a wire: the waveforms for different neurons are highly similar rather than orthogonal, the spikes emitted by a single neuron vary somewhat in amplitude, and it is not known in advance what the characteristic signal of each neuron is (or even how many neurons there are). 

One way to estimate how close we are likely to get to the above limits in practice is to extrapolate the state of the art in spike sorting.  At present, spike sorting algorithms operating on data from large-scale (\num{250}-\num{500} electrodes) densely spaced (\SI{\ca 30}{\micro\meter}) 2D multielectrode arrays can reliably identify and distinguish spikes from nearly all of the \num{200}-\num{300} retinal ganglion cells~\cite{marre12,pillow13} in a small patch of retina, and can also detect approximate cell locations through triangulation of spike amplitudes.  This represents a roughly 1:1 ratio of cells to electrodes.  If we assume that these algorithms scale linearly with the number of electrodes and perform roughly as well in 3D as in 2D, $N=\num{7.5e7}$ electrodes would presently be required to record from all mouse neurons, and could correspond to recording sites placed on the vertices of a \num{\ca 400 x 400 x 400} site cubic lattice with \SI{\ca 20}{\micro\meter} edge length.  In practice, 3D geometry will increase the number of spikes that register on each electrode, which could potentially worsen performance (by increasing noise and interference between spikes) or improve performance (by providing extra information for spike identification and triangulation).

It is difficult to assess the limits of the above techniques, because they presently succeed in detecting nearly all the cells in a region, and have not been extensively tested in cases where electrodes are sparser or cells are more dense.  Furthermore, most 3D techniques optimize for signal separability (e.g. hand-placed tetrodes), not number of recorded neurons, so it is somewhat difficult to extrapolate performance.  Optimistically, 3D simulations of neural activity suggest that \num{5}-\num{10} neurons per electrode may be distinguishable using current spike sorting algorithms~\cite{pedreira12,sahani99,camunas13}, although these simulations are based on a small number of electrodes (tetrodes or single units) and may not generalize to large-scale recording.  $\ca\num{10}$ neurons per electrode would imply $N=\num{7.5e6}$ electrodes to record from all neurons in the mouse brain, which could be accomplished by electrodes on a \SI{\ca 40}{\micro\meter} cubic lattice.

Future algorithmic improvements could enable sorting from more than $\ca\num{10}$ cells per electrode, but this becomes an increasingly challenging computational task, because spikes become weaker by an exponential factor, temporal overlaps between spikes become more common, and spikes from a given cell appear on a smaller number of electrodes – these factors are in part what lead to information-theoretic limit.  One simple estimate of a possible practical limit is the largest number of neurons that can be sorted without resolving temporally overlapping spikes – if the average neuron fires at \SI{\ca 5}{\hertz}  and spikes last \SI{\ca 2}{\milli\second}, then at most \num{100} neurons/electrode can be sorted without resolving overlaps.  Note that while some present-day algorithms can successfully resolve overlapping spikes,~\cite{marre12,pillow13,segev04,ge2011,prentice2011} they do so only in the case where electrodes are densely spaced and any given spike appears on many electrodes, such that spatial information can be used to resolve the overlap.  Resolving overlaps when spikes appear on one or a few channels (as would be the case when there are $>\num{100}$ neurons/electrode) is much more difficult 

Overall,  $\ca\num{100}$ cells/electrode may be taken as a very rough estimate of the ultimate limits of spike sorting, and would imply $N = \num{750,000}$ electrodes and thus an edge spacing of \SI{\ca 80}{\micro\meter} if a cubic lattice of recording sites were used.  However, we should not exclude the possibility of game changers which could somehow change the nature of the recorded data in a way that improves available information, for instance if CCD cameras were attached to electrode arrays and aided in the identification and localization of cells, or if directional information on the source of spikes could be obtained, for example through measuring the direction of gradients in voltage.  Such extra information would both circumvent information-theoretic limits and greatly improve spike sorting performance in practice.
