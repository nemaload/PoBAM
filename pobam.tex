...
It is possible to insert large numbers of probes throughout multiple brain areas without compromising function. For example, in one study in rats \cite{Ribeiro2004}, 96 electrodes of \SI{50}{\micro\meter} diameter were simultaneously inserted across four forebrain structures (cortex, thalamus, hippocampus and putamen). Note, however, that the total volume displacement in this experiment was below \SI{0.1}{\percent}.
...
Certain wavelengths of infrared light may pass through \SIrange{\ca 1}{2}{\milli\meter} of tissue without prohibitively strong attenuation~\cite{horton13,kobat09}, while depths of \SI{>500}{\micro\meter} cannot easily be reached in the visible range~\cite{Wilt2009}.
...
It may also be possible to power devices with applied magnetic fields (which are highly penetrant) using magneto-electric~\cite{Kitagawa2010, Priya2009, Yue2012, Fiebig2005} materials or via near-field inductive coupling. For the case of simple inductive coupling, a loop of \SI{10}{\micro\meter} diameter in an applied field of \SI{1}{\tesla} switching at \SI{1000}{\hertz} produces an induced electromotive force of only \SI{0.1}{\micro\volt}, however. Assuming a copper loop (\SI{\ca 17}{\nano\ohm\meter} resistivity) with \SI{1 x 1}{\micro\meter} cross-section and \SI{40}{\micro\meter} length (around the outer edge of the chip) gives a power of only \SI{\ca 15}{\femto\watt} associated with the induced current.
...
Due to Pauli exclusion, use of this technique requires unpaired electrons, which can be found e.g. in nitrogen vacancy diamond nano-crystals~\cite{horowitz12}, which are also highly sensitive to voltage~\cite{dolde11} and to magnetic fields~\cite{Hall2012}, and are amenable to optical control and readout of the spin state.
