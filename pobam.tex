...
Optical microscopy can currently record \num{\ca 100000} neurons at a \SI{1.25}{\second} timescale in behaving Zebrafish~\cite{ahrens13} (via light-sheet microscopy), or hundreds to thousands of neurons at a \SI{\ca100}{\milli\second} timescale in behaving mice~\cite{ziv13} (via two-photon microscopy). %Danio rerio (or whatever it's called)
...
\begin{figure}[htbp]
\centering
\caption{Four generalized neural recording modalities are studied here.}  \label{fig:modalities}
\subfloat[Optical]{\includegraphics[width=2.5in,trim=1.5mm 0mm 0mm 0mm,clip=true]{figs/optical_sketch_tc.eps}}
\hspace{0.1in}
\subfloat[Magnetic resonance]{\includegraphics[width=3.2in,trim=4mm 0mm 0mm 7mm,clip=true]{figs/mr_sketch_tc.eps}}
\\
\subfloat[Electrical]{\includegraphics[width=2.5in]{figs/electrode_sketch_tc.eps}}
\hspace{0.5in}
\subfloat[Molecular recording]{\includegraphics[width=2.7in,trim=0mm 0mm 3mm 0mm,clip=true]{figs/molecular_sketch_tc.eps}}
\end{figure}
...
We first treat the case in which there is a single channel for transmitting data out of the brain. The Shannon Capacity Theorem~\cite{cover06} sets the maximal bit rate for a channel (assuming white Gaussian noise) to
\[R\sub{max} = \BW \log_2 \left(1 + \SNR\right)\]
where \BW is the channel bandwidth and \SNR is the signal-to-noise ratio.
If there is only thermal noise the $\SNR = P/(N_0 \BW)$, where $N_0$ is the thermal noise power spectral density of $\kb T$ and $P=(\pathloss)P_0$ is the power of the transmitted signals $P_0$, which is weakened by path loss \pathloss. % remove "which is"
...
As discussed above, transmitting information through a \emph{single} channel imposes limits on bit rate, carrier frequency and input power.
It is conceivable to divide the data transmission burden over \emph{many} independent channels, however, i.e., over many pairs of transmitters and receivers, each operating at lower bandwidth (e.g., at radio frequencies).
...
In a typical MRI study, a strong static field ($B = \SIrange{1}{15}{\tesla}$) is applied to polarize nuclear spins (usually \textsuperscript{1}H), causing them to resonate at a field-dependent Larmor frequency, $f=\gamma B$, where $\gamma$ is the gyromagnetic ratio of the nucleus (\SI{42}{\MHz\per\tesla} for \textsuperscript{1}H). %fix to 267 [CODATA]
...
Such absorption is typically calculated through numerical solutions of the Maxwell Equations taking into account the precise geometry, tissue properties and applied fields for a particular experimental setup~\cite{collins04}.
...
\begin{figure}[htbp]
\caption{Key factors determining the spatiotemporal resolution of dynamic MRI imaging. For a given contrast agent concentration, one can determine the minimal temporal resolution from panel~\protect\subref{fig:mritemporal}, and from there calculate the minimal spatial resolution using panel~\protect\subref{fig:mrispatial}.}
\label{fig:mriresolution}
\centering
\subfloat[Temporal resolution and contrast agent concentration allowing \SI{>5}{\percent} contrast, for different classes of dynamic MRI contrast agent (reproduced from~\cite{shapiro06}, with permission).]{
\centering
\label{fig:mritemporal}
\includegraphics[width=3.5in]{figs/Fig6_temporal.eps}
}
\hspace{1mm}
\subfloat[Diffusion limited spatial resolution for water proton MRI as a function of temporal resolution.]{
\centering
\label{fig:mrispatial}
\includegraphics[width=3.5in]{figs/Fig6_spatial.eps}
}
\end{figure}
